...
/* ---------------- [2] 트리 정보 ---------------- */
static int tree_height(const ArrTree *t)      /* 레벨 수 (루트만 있으면 1) */
{
    int h = 0;
...
/* ---------------- [4] 메모리 사용량 ---------------- */
static void print_memory(const ArrTree *t)
{
    size_t elem  = sizeof t->a[0];
    size_t slots = (size_t)t->max_idx + 1;                /* 0번 칸(사용 안 함) ~ max_idx번 칸 */
    size_t empty = (size_t)t->max_idx - (size_t)t->n;     /* 1 ~ max_idx번 칸 중 빈 칸 */

    printf("실행 환경             : %zu비트 (포인터 크기 %zu 바이트)\n",
           sizeof(void *) * 8, sizeof(void *));
    printf("배열 원소 1개의 크기  : %zu 바이트\n", elem);
    printf("필요한 배열 칸 수     : %zu칸 (노드 %d칸 + 빈 칸 %zu칸 + 사용 안 하는 0번 칸 1칸)\n",
           slots, t->n, empty);
    printf("배열 사용량           : %zu 바이트\n", slots * elem);
    printf("공간 활용률           : %.1f%% (노드가 채운 칸 / 1~%d번 칸)\n",
           100.0 * t->n / t->max_idx, t->max_idx);
}
...
static long g_steps;     /* 배열 칸을 읽은 횟수 */

static char slot(const ArrTree *t, int i)
{
    g_steps++;
    return (i >= 1 && i < t->cap) ? t->a[i] : '\0';
}

/* target이 있는 배열 인덱스를 찾는다 (없으면 0, 같은 문자가 여럿이면 가장 앞 칸) */
static int find_index(const ArrTree *t, char target)
{
    for (int i = 1; i <= t->max_idx; i++)
        if (slot(t, i) == target) return i;
    return 0;
}

static void query_node(const ArrTree *t, char target)
{
    long find_steps, read_steps;
    char left, right, parent = '\0', sibling = '\0';
    int i;

    g_steps = 0;
    i = find_index(t, target);
    find_steps = g_steps;
    if (!i) {
        printf("노드 '%c'는 트리에 없습니다. (노드 찾기: %ld칸 확인)\n", target, find_steps);
        return;
    }
    ...
    printf("노드 '%c' (배열 인덱스 %d)\n", target, i);
    printf("  왼쪽 자식 : "); put_node(left);    printf("\n");
    printf("  오른쪽 자식: "); put_node(right);   printf("\n");
    printf("  부모      : "); put_node(parent);  printf("\n");
    printf("  형제      : "); put_node(sibling); printf("\n");
    printf("  [접근 횟수] 노드 찾기 %ld, 부모 위치 찾기 0 (i/2 계산), 자식·부모·형제 읽기 %ld\n",
           find_steps, read_steps);
}

static void query_loop(const ArrTree *t)
{
    char buf[MAX_INPUT];
